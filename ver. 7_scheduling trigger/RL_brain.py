...
class DeepQNetwork:
    def __init__(
            ...
            memory_size=500,
            batch_size=1000,
            #e_greedy_increment=None, #close
            e_greedy_increment=0.9/2000, #open it
            output_graph=False,
            double_q=True,
            save_model = 0,
            restore_model = 0
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            ...
            c_names, n_l1, n_l2, n_l3, n_l4, w_initializer, b_initializer = \
                ['eval_net_params', tf.GraphKeys.GLOBAL_VARIABLES], 40, 20, 10, 5, \
    ...
    def choose_action(self, observation, direction, run, lu, lun, ld, ldn):
        ...
        #         saver.restore(self.sess, 'results_100day_hour_26_01s/results_100day_hour_'+str(self.hour)+'/graph.chkp')
                # print('\nhour '+str(self.hour)+' model replace\n')


        if self.restore_model == 1:
            self.restore_model = 0
            saver = tf.train.Saver()
            saver.restore(self.sess, 'results_100day_24_86s/graph.chkp')
        ...
        if np.random.uniform() < self.epsilon:
            ...
            if observation[0][self.n_features-1] >= 0: #up rm
                for i in range(self.n_actions):
                    if (direction[i] == 0) or (direction[i] == 1 and observation[0][i] < observation[0][self.n_features-1]) or (direction[i] == 1 and observation[0][i] == observation[0][self.n_features-1] and (observation[0][self.n_actions+i] == 0 or observation[0][self.n_actions+i] > 5)) or (direction[i] == -1 and observation[0][i] > observation[0][self.n_features-1] and len(ld) == 0 and len(ldn[i]) == 0):
                        if q_temp == -1:
                ...
                for i in range(self.n_actions): # down rm
                    if (direction[i] == 0) or (direction[i] == -1 and observation[0][i] > (observation[0][self.n_features-1]*(-1))) or (direction[i] == -1 and observation[0][i] == (observation[0][self.n_features-1]*(-1)) and (observation[0][self.n_actions+i] == 0 or observation[0][self.n_actions+i] > 5)) or (direction[i] == 1 and observation[0][i] < (observation[0][self.n_features-1]*(-1)) and len(lu) == 0 and len(lun[i]) == 0):
                        if q_temp == -1:
                            q_temp = actions_value[0][i]
                            action = i
                        elif actions_value[0][i] < q_temp:
