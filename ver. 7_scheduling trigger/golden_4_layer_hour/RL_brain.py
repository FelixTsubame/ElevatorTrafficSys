...
class DeepQNetwork:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            ...
            with tf.variable_scope('l5'):
                ...
                self.q_eval = tf.nn.selu(tf.matmul(l4, w5) + b5)
        ...
        with tf.variable_scope('target_net'):
            ...
            with tf.variable_scope('l5'):
                w5 = tf.get_variable('w5', [n_l4, self.n_actions], initializer=w_initializer, collections=c_names)
                b5 = tf.get_variable('b5', [1, self.n_actions], initializer=b_initializer, collections=c_names)
                self.q_next = tf.nn.selu(tf.matmul(l4, w5) + b5)
    ...
    def choose_action(self, observation, direction, run):
        ...
        #     self.restore_model = 0
        #     saver = tf.train.Saver()
        #     saver.restore(self.sess, 'results_100day/graph.chkp')
        ...
        if np.random.uniform() < self.epsilon:
            ...
            if observation[0][self.n_features-1] >= 0: #up rm
                for i in range(self.n_actions):
                    if (direction[i] == 0) or (direction[i] == 1 and observation[0][i] < observation[0][self.n_features-1]) or (direction[i] == 1 and observation[0][i] == observation[0][self.n_features-1] and (observation[0][self.n_actions+i] == 0 or observation[0][self.n_actions+i] > 5)):
                        if q_temp == -1:
                ...
                for i in range(self.n_actions): # down rm
                    if (direction[i] == 0) or (direction[i] == -1 and observation[0][i] > (observation[0][self.n_features-1]*(-1))) or (direction[i] == -1 and observation[0][i] == (observation[0][self.n_features-1]*(-1)) and (observation[0][self.n_actions+i] == 0 or observation[0][self.n_actions+i] > 5)):
                        if q_temp == -1:
                            q_temp = actions_value[0][i]
                            action = i
                        elif actions_value[0][i] < q_temp:
