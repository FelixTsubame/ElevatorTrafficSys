		...
		if scheduling_trigger == 1 or rescheduling_up == 1:
			...
			while lu_id < len(lu):
				...
				if do == 0:
					...
					action_up = RL.choose_action(state_up, direction, run)
		...
		if scheduling_trigger == 1 or rescheduling_down == 1:
			...
			while ld_id < len(ld):
				...
				if do == 0:
					...
					action_down = RL.choose_action(state_down, direction, run)
					...
					if direction[action_down] == 0:
						...
						if position[action_down] < ld[ld_id][0]:
							...
							### 如果裡面有人要出來 ###
							if stc[action_up][position[action_down]] == 1:
								stc[action_up][position[action_down]] = 0
						elif position[action_down] > ld[ld_id][0]:
							direction[action_down] = -1
