...
def workday_rm_generator():
    ...
    fsm = []
    strm = []
    rm_num = poisson.rvs(mu=60, size=1)[0]
    for i in range(rm_num):
      temp_int = 0
      while True:
        temp_int = randint(0, 3600)
        if len(hour7_arrivaltime) != 0:
          if temp_int in hour7_arrivaltime:
            temp_int = temp_int
          else:
            break
        else:
          break
      hour7_arrivaltime.append(temp_int)
    ...
    fsm = []
    strm = []
    rm_num = poisson.rvs(mu=110, size=1)[0]
    for i in range(rm_num):
      temp_int = 0
      while True:
        temp_int = randint(3600, 7200)
        if len(hour8_arrivaltime) != 0:
          if temp_int in hour8_arrivaltime:
            temp_int = temp_int
          else:
            break
        else:
          break
      hour8_arrivaltime.append(temp_int)
      temp_fsm = choices(floor, weights_fsm)
      temp_strm = choices(floor, weights_strm)
      while temp_fsm == temp_strm:
        temp_fsm = choices(floor, weights_fsm)
    ...
    fsm = []
    strm = []
    rm_num = poisson.rvs(mu=70, size=1)[0]
    for i in range(rm_num):
      temp_int = 0
      while True:
        temp_int = randint(7200, 10800)
        if len(hour9_arrivaltime) != 0:
          if temp_int in hour9_arrivaltime:
            temp_int = temp_int
          else:
            break
        else:
          break
      hour9_arrivaltime.append(temp_int)
      temp_fsm = choices(floor, weights_fsm)
      temp_strm = choices(floor, weights_strm)
      while temp_fsm == temp_strm:
        temp_fsm = choices(floor, weights_fsm)
    ...
    fsm = []
    strm = []
    rm_num = poisson.rvs(mu=30, size=1)[0]
    for i in range(rm_num):
      temp_int = 0
      while True:
        temp_int = randint(10800, 14400)
        if len(hour10_arrivaltime) != 0:
          if temp_int in hour10_arrivaltime:
            temp_int = temp_int
          else:
            break
        else:
          break
      hour10_arrivaltime.append(temp_int)
      temp_fsm = choices(floor, weights_fsm)
      temp_strm = choices(floor, weights_strm)
      while temp_fsm == temp_strm:
        temp_fsm = choices(floor, weights_fsm)
    ...
    fsm = []
    strm = []
    rm_num = poisson.rvs(mu=20, size=1)[0]
    for i in range(rm_num):
      temp_int = 0
      while True:
        temp_int = randint(14400, 18000)
        if len(hour11_arrivaltime) != 0:
          if temp_int in hour11_arrivaltime:
            temp_int = temp_int
          else:
            break
        else:
          break
      hour11_arrivaltime.append(temp_int)
      temp_fsm = choices(floor, weights_fsm)
      temp_strm = choices(floor, weights_strm)
      while temp_fsm == temp_strm:
        temp_fsm = choices(floor, weights_fsm)
    ...
    for i in range(rm_num):
      temp_int = 0
      while True:
        temp_int = randint(18000, 21600)
        if len(hour12_arrivaltime) != 0:
          if temp_int in hour12_arrivaltime:
            temp_int = temp_int
          else:
            break
        else:
          break
      hour12_arrivaltime.append(temp_int)
      temp_fsm = choices(floor, weights_fsm)
      temp_strm = choices(floor, weights_strm)
      while temp_fsm == temp_strm:
        temp_fsm = choices(floor, weights_fsm)
    ...
    fsm = []
    strm = []
    rm_num = poisson.rvs(mu=140, size=1)[0]
    for i in range(rm_num):
      temp_int = 0
      while True:
        temp_int = randint(21600, 25200)
        if len(hour13_arrivaltime) != 0:
          if temp_int in hour13_arrivaltime:
            temp_int = temp_int
          else:
            break
        else:
          break
      hour13_arrivaltime.append(temp_int)
      temp_fsm = choices(floor, weights_fsm)
      temp_strm = choices(floor, weights_strm)
      while temp_fsm == temp_strm:
        temp_fsm = choices(floor, weights_fsm)
    ...
    for i in range(rm_num):
      temp_int = 0
      while True:
        temp_int = randint(25200, 28800)
        if len(hour14_arrivaltime) != 0:
          if temp_int in hour14_arrivaltime:
            temp_int = temp_int
          else:
            break
        else:
          break
      hour14_arrivaltime.append(temp_int)
      temp_fsm = choices(floor, weights_fsm)
      temp_strm = choices(floor, weights_strm)
      while temp_fsm == temp_strm:
        temp_fsm = choices(floor, weights_fsm)
    ...
    for i in range(rm_num):
      temp_int = 0
      while True:
        temp_int = randint(28800, 32400)
        if len(hour15_arrivaltime) != 0:
          if temp_int in hour15_arrivaltime:
            temp_int = temp_int
          else:
            break
        else:
          break
      hour15_arrivaltime.append(temp_int)
      temp_fsm = choices(floor, weights_fsm)
      temp_strm = choices(floor, weights_strm)
      while temp_fsm == temp_strm:
        temp_fsm = choices(floor, weights_fsm)
    ...
    for i in range(rm_num):
      temp_int = 0
      while True:
        temp_int = randint(32400, 36000)
        if len(hour16_arrivaltime) != 0:
          if temp_int in hour16_arrivaltime:
            temp_int = temp_int
          else:
            break
        else:
          break
      hour16_arrivaltime.append(temp_int)
      temp_fsm = choices(floor, weights_fsm)
      temp_strm = choices(floor, weights_strm)
      while temp_fsm == temp_strm:
        temp_fsm = choices(floor, weights_fsm)
    ...
    fsm = []
    strm = []
    rm_num = poisson.rvs(mu=80, size=1)[0]
    for i in range(rm_num):
      temp_int = 0
      while True:
        temp_int = randint(36000, 39600)
        if len(hour17_arrivaltime) != 0:
          if temp_int in hour17_arrivaltime:
            temp_int = temp_int
          else:
            break
        else:
          break
      hour17_arrivaltime.append(temp_int)
      temp_fsm = choices(floor, weights_fsm)
      temp_strm = choices(floor, weights_strm)
      while temp_fsm == temp_strm:
        temp_fsm = choices(floor, weights_fsm)
    ...
    for i in range(rm_num):
      temp_int = 0
      while True:
        temp_int = randint(39600, 43200)
        if len(hour18_arrivaltime) != 0:
          if temp_int in hour18_arrivaltime:
            temp_int = temp_int
          else:
            break
        else:
          break
      hour18_arrivaltime.append(temp_int)
      temp_fsm = choices(floor, weights_fsm)
      temp_strm = choices(floor, weights_strm)
      while temp_fsm == temp_strm:
        temp_fsm = choices(floor, weights_fsm)
    ...
    for i in range(rm_num):
      temp_int = 0
      while True:
        temp_int = randint(43200, 46800)
        if len(hour19_arrivaltime) != 0:
          if temp_int in hour19_arrivaltime:
            temp_int = temp_int
          else:
            break
        else:
          break
      hour19_arrivaltime.append(temp_int)
      temp_fsm = choices(floor, weights_fsm)
      temp_strm = choices(floor, weights_strm)
      while temp_fsm == temp_strm:
        temp_fsm = choices(floor, weights_fsm)
    ...
    for i in range(rm_num):
      temp_int = 0
      while True:
        temp_int = randint(46800, 50400)
        if len(hour20_arrivaltime) != 0:
          if temp_int in hour20_arrivaltime:
            temp_int = temp_int
          else:
            break
        else:
          break
      hour20_arrivaltime.append(temp_int)
      temp_fsm = choices(floor, weights_fsm)
      temp_strm = choices(floor, weights_strm)
      while temp_fsm == temp_strm:
        temp_fsm = choices(floor, weights_fsm)
    ...
    fsm = []
    strm = []
    rm_num = poisson.rvs(mu=5, size=1)[0]
    for i in range(rm_num):
      temp_int = 0
      while True:
        temp_int = randint(50400, 54000)
        if len(hour21_arrivaltime) != 0:
          if temp_int in hour21_arrivaltime:
            temp_int = temp_int
          else:
            break
        else:
          break
      hour21_arrivaltime.append(temp_int)
      temp_fsm = choices(floor, weights_fsm)
      temp_strm = choices(floor, weights_strm)
      while temp_fsm == temp_strm:
        temp_fsm = choices(floor, weights_fsm)
